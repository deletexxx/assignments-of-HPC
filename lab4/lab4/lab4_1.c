/* File:     lab4_1.c
 *
 * Purpose:  Use a layer of openmp to implement matrix multiplication
 *
 * Compile:  gcc -o lab4_1 lab4_1.c -lpthread 
 *
 * Run:      ./lab4_1 <num> <m> <k> <n>
...
int main(int argc, char *argv[]){
    ...
    start_time = clock();
    #pragma omp parallel for private(temp_c) shared(A, B, C)
        for(int i=0; i<m; ++i){
            for(int j=0; j<k; ++j){
                temp_c = 0;
