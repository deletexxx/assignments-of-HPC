/* File:     lab4_1_2.c
 *
 * Purpose:  Use two layers of openmp to implement matrix multiplication
 *
 * Compile:  gcc -o lab4_1_2 lab4_1_2.c -lpthread 
 *
 * Run:      ./lab4_1_2 <num> <m> <k> <n>
...
int main(int argc, char *argv[]){
    ...
    start_time = clock();
    #pragma omp parallel for shared(A, B, C) private(temp_c)
        for(int i=0; i<m; ++i){
            for(int j=0; j<k; ++j){
                temp_c = 0;
                #pragma omp parallel for firstprivate(i,j) shared(A, B, C) reduction(+:temp_c)
