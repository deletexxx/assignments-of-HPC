/* File:     lab4_2_dyna.c
 *
 * Purpose:  Use the dynamic scheduling method to optimize 
 *           the general matrix multiplication implemented by openmp
 *
 * Compile:  gcc -o lab4_2_dyna lab4_2_dyna.c -fopenmp
 *
 * Run:      ./lab4_2_dyna <num> <m> <k> <n>
...
int main(int argc, char *argv[]){
    ...
    start_time = clock();
    #pragma omp parallel for private(temp_c) shared(A, B, C) schedule(dynamic)
        for(int i=0; i<m; ++i){
            for(int j=0; j<k; ++j){
                temp_c = 0;
                #pragma omp parallel for firstprivate(i,j) shared(A, B, C) reduction(+:temp_c) schedule(dynamic)
