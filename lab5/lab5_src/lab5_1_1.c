 ...
 *           change step(),write a new step(),and write the original function step() as step_(),so original function step() is not called in the program
 *
 * Compile:  gcc -o lab5_1_1  -o lab5_1_1 -lpthread -lm
 *
 * Run:      ./lab5_1_1
 *
...
for (j = thread_assign->start; j < thread_assign->end; j=j+thread_assign->increment )
  {
    jw = j * thread_assign->mj;
    ja  = jw;
    jb  = ja;
    jc  = j * (thread_assign->mj * 2);
    jd  = jc;

    wjw[0] = thread_assign->w[jw*2+0]; 
    wjw[1] = thread_assign->w[jw*2+1];

    if ( thread_assign->sgn < 0.0 ) 
    {
      wjw[1] = - wjw[1];
    }

    for (int k = 0; k < thread_assign->mj; k++ )
    {
      thread_assign->c[(jc+k)*2+0] = thread_assign->a[(ja+k)*2+0] + thread_assign->b[(jb+k)*2+0];
      thread_assign->c[(jc+k)*2+1] = thread_assign->a[(ja+k)*2+1] + thread_assign->b[(jb+k)*2+1];

      ambr = thread_assign->a[(ja+k)*2+0] - thread_assign->b[(jb+k)*2+0];
      ambu = thread_assign->a[(ja+k)*2+1] - thread_assign->b[(jb+k)*2+1];

      thread_assign->d[(jd+k)*2+0] = wjw[0] * ambr - wjw[1] * ambu;
      thread_assign->d[(jd+k)*2+1] = wjw[1] * ambr + wjw[0] * ambu;
    }
  }
...
void parallel_for_step(int start, int end, int increment, void *(*functor)(void*), void *arg , int num_threads){
    ...
    for(int i=0; i<num_threads; ++i){
        ...
        thread_assign[i].a =  first->a;
        thread_assign[i].b =  first->b;
        thread_assign[i].c =  first->c;
        thread_assign[i].d =  first->d;
    ...
    ret = gettimeofday(&newVal, NULL);
    unsigned long diff = (newVal.tv_sec * Converter + newVal.tv_usec) - (val.tv_sec * Converter + val.tv_usec);
    // printf("creat pthread cost time :usec --- %ld\n",  diff - time);
  ...
  double d[], double w[], double sgn )
{
  ...
  parallel_for_step(0,lj,1,step_parallel,&first,num_thread);
...
void step_ ( int n, int mj, double a[], double b[], double c[],
  double d[], double w[], double sgn )
...
*/
{
  ...
  ret = gettimeofday(&newVal, NULL);
   unsigned long diff = (newVal.tv_sec * Converter + newVal.tv_usec) - (val.tv_sec * Converter + val.tv_usec);
    ...
    // printf("serial step cost time: usec --- %ld\n", diff );
  return;
}
/******************************************************************************/
